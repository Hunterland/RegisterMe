...
router.post("/users", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Ocorreu um erro ao cadastrar o usu�rio" });
  }
});
...
router.get("/users", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Ocorreu um erro ao listar os usu�rios" });
  }
});
...
router.put("/users/:id", async (req, res) => {
  try {
    ...
    if (idade <= 18) {
      return res
        .status(400)
        .json({ error: "Usu�rio deve ter mais de 18 anos" });
    }
    ...
    return res
      .status(500)
      .json({ error: "Ocorreu um erro ao editar o usu�rio" });
  }
});
...
router.delete("/users/:id", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Ocorreu um erro ao remover o usu�rio" });
  }
});
